    ...
    <article className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group">
      ...
      {imagen && (
        <div className="relative h-52 w-full overflow-hidden">
          ...
          />
          {fuente === "INSTAGRAM" && (
            <span className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-pink-600 rounded-full p-1.5">
              <Camera size={14} />
            </span>
          )}
        </div>
      ...
      <div className="p-5">
        ...
        <h3 className="font-bold text-gray-900 text-lg mb-2 line-clamp-2">{titulo}</h3>

        {contenido && (
          <p className="text-gray-500 text-sm leading-relaxed line-clamp-3 mb-4">{contenido}</p>
        )}
